...
export const getMachineList = (params) => {
  return request({
    url: '/run', // 核心：和后端RunController的@GetMapping("/run")完全对应
    method: 'get',
    params: params // 传递分页参数（page、size）
  });
};

// ✅ 预约跑步机（格式统一）
export const reserveMachine = (id) => {
  return request({
    url: `/run/${id}/reserve`, // 和后端 @PostMapping("/run/{id}/reserve") 对应
    method: 'post'
  });
};
